...
export async function enviarNotificacionesResultado({ loteria, fecha, premio, descripcion }) {
  ...
  const selectCols = 'id, email, notif_correo, notif_push, notif_solo_ganadores, push_subscription, push_notifications, push_notification';
  const { data, error } = await supabaseAdmin.from('profiles').select(selectCols);
  if (error) {
    usuariosError = error;
    const fallback = await supabaseAdmin.from('profiles').select('id, email, notif_correo, notif_push, notif_solo_ganadores, push_subscription, push_notifications, push_notification');
    if (fallback.error) {
  ...
  const promises = usuarios.map(async (usuario) => {
    ...
    const subscription = usuario.push_subscription || usuario.push_notifications || usuario.push_notification;
    if (usuario.notif_push && subscription) {
